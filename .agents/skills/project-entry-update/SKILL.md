...
Input: a project id (`<project-id>`, the markdown filename without `.md`) and its source repository, resolved relative to the workspace (sibling folder `..` or `$env:REPOS_DIR`). Never hardcode absolute paths.

## Steps

1. **Read the source.** Inspect the repository's `README.md` and manifest (`package.json`, `Cargo.toml`, `.csproj`, `pyproject.toml`) and its recent history (`git log -n 10 --oneline`). Note version bumps, architectural shifts, new features and key learnings.
...
4. **Set `lastUpdated`** to today's date (`YYYY-MM-DD`) in both files whenever the entry's content changes. It is shown on the case-study page, so a stale date misleads readers. Do not touch it for pure formatting changes.
5. **No country flag emojis** anywhere in the entry.
